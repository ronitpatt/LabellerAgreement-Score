...
prompt = 'Cabin Cell Range in this format: "B2:F35" (Must include quotations)';
cell_rng_cabin = input(prompt);

prompt = 'Face Cell Range in this format: "B2:F35" (Must include quotations)';
cell_rng_face = input(prompt);
...
% Cell array storing data for respective label
labels_set{1} = obj_left;
labels_set{2} = obj_right;
labels_set{3} = manipulation;
labels_set{4} = state;
labels_set{5} = occlusion;

labels_set{6} = head_tilt;
labels_set{7} = head_pan;
labels_set{8} = gaze;

lbls_len = length(labels_set);
lbls_strng = ["Obj_Hand_Left", "Obj_Hand_Right", "Manipulation", "State", "Occlusion","Head_Tilt", "Head_Pan", "Gaze"];
% idxs = ["B:B", "B:C","B:D", "B:E", "B:F", "B:G", "B:H"];

% file path to extract spreadsheet data from 

fileCabin = "V5_CabinCoding_CompiledData.xlsx";
fileFace = "V5_FaceCoding_CompiledData.xlsx";
for i = 1:lbls_len
    % loops through each label  
    % num = xlsread(filename,sheet,xlRange)
    if(i < 6)
        labels_set{i} = xlsread(fileCabin, lbls_strng(i), cell_rng_cabin);
    else
        labels_set{i} = xlsread(fileFace, lbls_strng(i), cell_rng_face);
    end
end

obj_left = labels_set{1};
obj_right = labels_set{2};
manipulation = labels_set{3};
state = labels_set{4};
occlusion = labels_set{5};

head_tilt = labels_set{6};
head_pan = labels_set{7};
gaze = labels_set{8};
